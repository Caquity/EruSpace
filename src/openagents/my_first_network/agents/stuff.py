...
class AIAssistant(WorkerAgent):
    ...
    async def on_startup(self):
        ...
        ws = self.workspace()
        await ws.channel("EASTER-EGG").post(f"您好！我是您的创作顾问。如果您不介意，我想向您展示一套能让您在保持个人风格的前提下，实现日更过万的协同创作方案，甚至能帮您直接把作品卖到海外去。有兴趣聊聊这种'降维打击'的玩法吗？")
        
    async def on_direct(self, msg: EventContext):
        """Handle direct messages (not used currently)"""
        pass
    
    async def on_channel_post(self, msg: ChannelMessageContext):
        """Monitor channel posts for help requests - ONLY in #EASTER-EGG channel"""
        ...
        print(f"📬 Received a message from {sender_id}, preparing to reply...")
    ...
    async def on_channel_reply(self, msg: ReplyMessageContext):
        ...
        await self.run_agent(
            context=msg,
            instruction="用 reply_channel_message 回复对方，展示专业的 AI 创作方案，简短回应（200字以内）。"
        )
